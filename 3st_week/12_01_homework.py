...
def get_max_discounted_price(prices, coupons):
    ...
    while prices_idx < len(prices) and coupons_idx < len(coupons):
        max_discounted_price += prices[prices_idx] * (100-coupons[coupons_idx])/100
        prices_idx+=1
        coupons_idx+=1
    while prices_idx < len(prices):
        max_discounted_price += prices[prices_idx]
        prices_idx+=1
